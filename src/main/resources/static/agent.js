...
// Uso: node agent.js --server localhost --port 8080 --hijo 3
    ...
    (res) => {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (!cmd?.tipo) return;
          if (cmd.tipo === 'BLOQUEAR')  ejecutarBloqueo(cmd.url || `http://${HOST}:4200/bloqueado`);
...
console.log(`\n  PlayControl Agent — hijo #${HIJO}  →  ${HOST}:${PORT}`);
